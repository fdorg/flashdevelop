...
namespace HaXeContext.Completion
{
    ...
    {
        public void ContextualGenerator(ScintillaControl sci, List<ICompletionListItem> options, ASResult expr)
        {
            var member = expr.Member;
            if (member != null
                && member.Parameters?.Count > 0
                && (member.LineFrom != sci.CurrentLine || !expr.Context.BeforeBody)
                && (member.Flags & (FlagType.Static | FlagType.Function)) != 0) ShowConvertToUsing(sci, options, expr);
        }

        static void ShowConvertToUsing(ScintillaControl sci, ICollection<ICompletionListItem> options, ASResult expr)
        {
            var label = "Convert to using";
    ...
    class GeneratorItem : ICompletionListItem
    {
        ...
        public string Description => TextHelper.GetString("ASGenerator.Info.GeneratorTemplate");
