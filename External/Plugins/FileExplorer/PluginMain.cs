    ...
    public class PluginMain : IPlugin
    {
        ...
        private void ExploreDirectory(string path)
        {
            try
            {
                ...
                int start = explorer.StartsWithOrdinal("\"") ? explorer.IndexOfOrdinal("\"", 2) : 0;
                int p = explorer.IndexOfOrdinal(" ", start);
                if (!path.StartsWithOrdinal("\"")) path = "\"" + path + "\"";
                // Start the process...
        ...
        private void PromptHere(string path)
        {
            try
            {
                ...
                int start = cmd.StartsWithOrdinal("\"") ? cmd.IndexOfOrdinal("\"", 2) : 0;
                int p = cmd.IndexOfOrdinal(" ", start);
                if (path.StartsWithOrdinal("\"") && path.Length > 2) path = path.Substring(1, path.Length - 2);
                // Start the process...
