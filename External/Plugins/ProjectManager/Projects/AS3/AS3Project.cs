...
namespace ProjectManager.Projects.AS3
{
    public class AS3Project : Project
    {
        ...
        {
            if (!FileInspector.IsSwc(path) && !FileInspector.isDirectory(path)) return base.IsLibraryAsset(path);
        ...
        {
            if (!FileInspector.IsSwc(path) && !FileInspector.isDirectory(path)) return base.GetAsset(path);
        ...
        {
            if (!FileInspector.IsSwc(path) && !FileInspector.isDirectory(path)) base.SetLibraryAsset(path, isLibraryAsset);
            else
            {
